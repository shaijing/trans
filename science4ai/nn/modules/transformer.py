...
class EncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.norm1 = LayerNorm(d_model, **factory_kwargs)
    ...
    def _sa_block(self, x, atten_mask):
        x = self.self_attn(x, x, x, atten_mask)
...
class DecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.self_attn = MultiHeadedAttention(d_model, nhead, dropout, **factory_kwargs)
        # 上下文注意力机制
        self.multihead_attn = MultiHeadedAttention(d_model, nhead, dropout, **factory_kwargs)
        self.feed_forward = PositionwiseFeedForward(d_model, dim_feedforward, **factory_kwargs)
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        super().__init__()
        encoder_layer = EncoderLayer(d_model, nhead, dim_feedforward, dropout, norm_first, **factory_kwargs)
        ...
        decoder_layer = DecoderLayer(d_model, nhead, dim_feedforward, dropout, norm_first, **factory_kwargs)
